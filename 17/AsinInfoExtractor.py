...
r.set(str(cNumber)+"_"+str(date)+"_STG_1_progress", 0)
r.set(str(cNumber) + "_" + str(date) + "_STG_1_total", 0)
r.set(str(cNumber) + "_" + str(date) + "_STG_1_total_MissedID", 0)
r.set(str(cNumber) + "_" + str(date) + "_STG_1_progress_MissedID", 0)
...
def Asinator_ping(df,region,spliteddata,outputFile,flag=False):
    try:
        ...
        r.incrby(str(cNumber) + "_" + str(date) + "_STG_1_total", len(total_retailer_ids))
        for inner_data in spliteddata:
            res_data,total_time=asin_req(inner_data, region)
            if res_data=="":
               Log_writer("OCR_ErrorLog_" + str(cNumber) + ".log", cNumber, "max retry skipped", "445","Asinator Resumed")
               continue
            if flag:
            ...
            if res_data.status_code==200:
                ...
                for index,data in enumerate(res_data_json):
                    ...
                    if data["title"] is None:
                        ...
                        if data["manufacturer"] == "none" or data["manufacturer"] == "None" or data[
                ...
                pass
            r.incrby(str(cNumber) + "_" + str(date) + "_STG_1_progress", len(inner_data))
